...
def load_postgres_instance_datasource_asset(context):
    ...
    if asset_name in list_asset_names:
        ...
        table_asset = datasource.add_table_asset(
            name=asset_name, table_name=asset_table_name, schema_name="raw"
        )
        # add_query_asset
...
def load_csv_datasource_asset_raw(
    context, datasource_name: str, asset_name: str, layer_name="raw"
):
    """Cria ou carrega o Datasource, o Data Asset e o objeto Batch Request para a validação
    com o Great Expectations. É a primeira etapa do workflow, sendo a definição dos dados
    que serão validados.
    ...
    if datasource == None:
        datasource = context.sources.add_pandas_filesystem(
            datasource_name, base_directory="../data"
        )
    ...
    if asset_name in list_asset_names:
        ...
        table_asset = datasource.add_csv_asset(
            asset_name, batching_regex=file_data_regex
        )
...
def load_df_datasource_asset_trusted(
    context, df, datasource_name: str, asset_name: str
):
    """Cria ou carrega o Datasource, o Data Asset e o objeto Batch Request para a validação
    com o Great Expectations. É a primeira etapa do workflow, sendo a definição dos dados
    que serão validados.
